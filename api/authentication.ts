import { flaqAxios, getBaseUrl } from "./config/axios"
...
if(error instanceof AxiosError){
    if(error.response?.status === 400){
        return {status: Status.BadRequest, data: {reason: FailedAuthenticationReason.InvalidCode, message: "Invalid code"}}
    }
}
return {status: Status.Unauthorized, data: {reason: FailedAuthenticationReason.Unkown, message: "Unknown error occured."}}
}
}